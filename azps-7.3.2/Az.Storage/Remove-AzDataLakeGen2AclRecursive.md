...
> [!NOTE]
>Ini adalah versi dokumentasi kami sebelumnya. Silakan lihat [versi terbaru](/powershell/module/az.storage/remove-azdatalakegen2aclrecursive) untuk informasi terbaru.
...
## DESCRIPTION
Cmdlet **Remove-AzDataLakeGen2AclRecursive** menghapus ACL secara rekursif pada jalur yang ditentukan. Entri ACL dalam ACL asli, yang memiliki AccessControlType, DefaultScope, dan EntityId yang sama dengan entri ACL input (bahkan dengan izin yang berbeda) yang akan dihapus lbe.

## EXAMPLES

### Contoh 1: Hapus ACL secara rekurtif pada akar directiry dari filesystem
```
...
Perintah ini terlebih dahulu membuat objek ACL dengan 2 entri acl, lalu menghapus ACL secara rekursif pada direktori akar sistem file.

### Contoh 2: Hapus ACL secara rekurtif di direktori
```
...
### Contoh 3: Remove ACL recursively chunk by chunk
```
$token = $null
...
Skrip ini akan menghapus ACL secara reskursif pada bagian direktori menurut bagian, dengan ukuran potongan sebagai BatchSize * MaxBatchCount. Ukuran potongan adalah 50000 dalam skrip ini.

### Contoh 4: Hapus ACL secara rekurtif di direktori dan ContinueOnFailure, lalu lanjutkan dari kegagalan satu per satu
```
...
Perintah ini terlebih dahulu menghapus ACL secara berulang ke direktori dengan ContinueOnFailure, dan beberapa item gagal, lalu melanjutkan item yang gagal satu per satu.
