...
Konteks Azure adalah objek PowerShell yang mewakili langganan aktif untuk dijalankan, dan informasi autentikasi yang diperlukan untuk tersambung ke cloud Azure. Dengan konteks Azure, Azure PowerShell anda tidak perlu lagi mengautentikasi akun setiap kali beralih langganan. Konteks Azure terdiri dari:
...
Untuk menghindari beralih konteks untuk seluruh sesi PowerShell, Azure PowerShell semua perintah dapat dijalankan berdasarkan konteks yang ada dengan `AzContext` parameter:
...
* Masuk menggunakan `-Scope Process` dengan `Connect-AzAccount` .
  ...
  Konteks Azure yang dikembalikan sebagai bagian dari proses masuk  ini hanya valid untuk sesi saat ini dan tidak akan disimpan secara otomatis, terlepas dari pengaturan Azure PowerShell simpan otomatis konteks.
