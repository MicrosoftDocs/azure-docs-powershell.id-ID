...
## DESKRIPSI
Cmdlet Get-AzEventGridSubscription mendapatkan detail langganan Kisi Kejadian yang ditentukan, atau daftar semua langganan Kisi Kejadian di langganan Azure saat ini atau grup sumber daya.
...
Membuat daftar 15 langganan acara kawasan pertama (jika ada) yang dibuat di bawah grup sumber daya \` MyResourceGroupName di lokasi \` tertentu westus2 yang memenuhi $odataFilter sumber \` \` daya. Jika hasil lainnya tersedia, $result. NextLink tidak akan $null. Untuk mendapatkan halaman langganan acara berikutnya, pengguna diharapkan untuk melakukan panggilan ulang Get-AzEventGridSubscription dan menggunakan hasilnya. NextLink diperoleh dari panggilan sebelumnya. Penelepon harus berhenti saat hasil. NextLink menjadi $null.
